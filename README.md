# Solana Token Creator

A powerful Solana token generator and simulator built in Python. This tool allows users to easily create their own tokens on the Solana blockchain and provides simulation capabilities for testing.

## Features
- Complete Solana Token Generation logic
- Built-in simulation environment (`simulator.py`)
- Configurable environment options

## Acknowledgments & Inspiration

This project was built leveraging concepts, architectures, and tutorials from the following incredible resources. They were instrumental in the development of this project:

- [YouTube Tutorial - Solana Token Creation](https://www.youtube.com/watch?v=L4ASwqLZVV0)

*These resources served as foundational inspiration, highlighting best practices and robust methods for interacting with the Solana blockchain which were implemented in this tool.*

## Setup & Execution
1. Install dependencies from `requirements.txt`:
   ```bash
   pip install -r requirements.txt
   ```
2. Set up your `.env` file with appropriate keys.
3. Run `main.py` or the `simulator.py` to get started!
